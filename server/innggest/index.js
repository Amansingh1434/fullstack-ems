...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Wait for 9 hours
    await step.sleepUntil("wait-foe-the-9-hours",new Date(new Date().getTime()+9*60*60*1000))
...
const  attendanceReminderCron = inngest.createFunction(
  ...
  async ({  step })=>{
    ...
    // step 4: Get employee IDs who aloready checked in today
    const checkedInIds = await step.run("get-checked-in-ids",async()=>{
        const attendances = await Attendance.find({
            date:{$get:new Date(today.startUTC),$lt:new Date(today.endUTC)}
        }).lean();
