...
const AttendanceStats = ({history}) => {
  // const totalPresent = history.filter((h)=>h.status==="PREAENT" | h.status==="LATE").length;
...
const totalLate = safeHistory.filter(
  (h) => h.status === "LATE"
).length;
  const stats=[
    {lable:"Day Present" , value:totalPresent,icon:CalendarIcon},
  ...
  return (
    <div className=' grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
      {stats.map((s)=>(
        <div key={s.lable} className=' card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group'>
